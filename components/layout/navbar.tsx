...
import { Menu, Palette, FileText, TestTube, ArrowRight, X } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Sheet, SheetContent, SheetTrigger, SheetClose } from "@/components/ui/sheet"

const navItems = [
  { name: "Inicio", href: "/", icon: Palette },
  { name: "Documentación", href: "/docs", icon: FileText },
  { name: "Probar Modelo", href: "/demo", icon: TestTube },
]
  ...
  return (
    <motion.header
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        isScrolled ? "bg-white/80 backdrop-blur-md border-b border-gray-200/50 shadow-sm" : "bg-transparent"
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            <SheetTrigger asChild>
              <Button variant={"ghost"} size="icon" className="md:hidden hover:bg-transparent">
                <Menu className="h-6 w-6 text-gray-700 dark:text-gray-200 transition-colors duration-300 hover:text-blue-600 dark:hover:text-blue-400" />
              </Button>
            </SheetTrigger>
            <SheetContent
              side="right"
              className="w-[320px] sm:w-[380px] p-0 border-0 bg-gradient-to-br from-white via-blue-50 to-purple-50"
            >
              {/* Header with backdrop blur effect */}
              <div className="relative p-6 bg-white/80 backdrop-blur-md border-b border-gray-200/50">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="absolute top-4 right-4 z-50">
                    <SheetClose asChild>
                      <Button  size="icon" className="text-gray-700 bg-transparent hover:bg-transparent hover:transparent">
                        <X className="h-5 w-5" />
                      </Button>
                    </SheetClose>
                  </div>
                ...
                <div className="flex-1 p-6">
                  <div className="space-y-2">
                    ...
                    <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                      <Link
                        href="/"
                        className="flex items-center space-x-4 p-4 rounded-2xl bg-white/60 backdrop-blur-sm border border-white/20 shadow-sm hover:shadow-md hover:bg-white/80 transition-all duration-300 group"
                  ...
                  <motion.div className="mt-8" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                    <Link href="/demo" className="block" onClick={() => setIsMobileMenuOpen(false)}>
